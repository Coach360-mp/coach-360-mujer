...
export default function HomePage() {
  ...
  async function checkUser() {
    ...
    if (session?.user) {
      ...
      // Solo bypass del landing si ya completó onboarding — va al dashboard.
      // Si tiene sesión pero onboarding incompleto, mostrar landing igual:
      // que decida cuándo continuar clickeando "Comenzar".
  ...
  // Si ya hay sesión activa, "Comenzar" va directo a /onboarding sin abrir modal.
  // Si no, abre el modal de signup.
  ...
  const handleSignup = async (e) => {
    ...
    // Si Supabase tiene "Confirm email" ON, data.session es null
    if (!data.session) {
