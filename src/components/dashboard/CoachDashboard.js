...
export default function CoachDashboard({ vertical = 'mujer' }) {
  const config = getCoachConfig(vertical)
  ...
  async function guardarRitual(accionesActuales) {
    ...
    try {
      const res = await fetch('/api/daily-checkin', {
        ...
        body: JSON.stringify({ userId: user.id, acciones: accionesActuales, vertical: config.vertical }),
      })
  ...
  useEffect(() => {
    if (testResult && perfil && !perfil?.popup_post_test_visto?.[vertical]) {
  ...
  async function cerrarPopupPostTest(navegarDespues) {
    ...
    if (user) {
      const nuevoVisto = { ...(perfil?.popup_post_test_visto || {}), [vertical]: true }
      try {
  ...
  const checkUser = async () => {
    ...
    if (!user) { router.push(config.landingPath); return }
    setUser(user)
    ...
    setChatMsgs([{ r: 'a', t: config.saludoFresco(perfil?.nombre) }])
    const { data: t } = await supabase.from('tests').select('*').eq('activo', true).eq('vertical', config.vertical).order('orden')
    const { data: h } = await supabase.from('templates').select('*').eq('activo', true).eq('vertical', config.vertical).order('orden')
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); router.push(config.landingPath) }
  ...
  const answerQuestion = async (value) => {
    ...
    if (testStep + 1 < testQuestions.length) { setTestStep(testStep + 1) }
      ...
      const profile = perfilData ? {
        ...
        icono: '✦',
      } : { nombre: 'Explorador', desc: 'Tienes una combinación única.', descLarga: `${coachName} te va a ayudar a interpretarlo.`, icono: '✦', color: '#d4af37', colorBg: 'rgba(212,175,55,0.12)', fortalezas: [], sombras: [], recomendaciones: [], promptClara: `Acabo de hacer el test "${activeTest?.titulo}". Quiero hablarlo contigo.` }
      ...
      if (user) {
        ...
        try {
          await supabase.from('user_context').upsert({
            user_id: user.id, vertical: config.vertical, context_key: 'ultimo_test_resultado',
        ...
        try {
          fetch('/api/emails/resultado-test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, vertical: config.vertical, testTitulo: activeTest.titulo, perfilResultado: profile.nombre }) })
  ...
  const completarPrimeraSesion = async () => {
    if (user) {
      ...
      if (sesionAnimo) {
        await supabase.from('daily_checkins').insert({ user_id: user.id, vertical: config.vertical, mood: sesionAnimo, created_at: new Date().toISOString() })
      }
      const currentCoach = coaches[perfil?.plan_actual || 'free']
      setChatMsgs([{ r: 'a', t: config.saludoConocida(perfil?.nombre) }])
    }
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px 40px' }}>
        ...
        {sesionPaso === 0 && !sesionMostrarTour && (
          <div style={{ textAlign: 'center', maxWidth: 360 }}>
            <img src={coachImg} alt={coachName} style={{ width: 100, height: 100, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--gold-light)', marginBottom: 24 }} />
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, marginBottom: 12, color: 'var(--text)' }}>Hola, {perfil?.nombre || 'bienvenida'} ✦</h1>
            <p style={{ fontSize: 16, color: 'var(--text-light)', lineHeight: 1.6, marginBottom: 8 }}>Soy {coachName}, tu coach en Coach 360.</p>
          ...
          <div style={{ textAlign: 'center', maxWidth: 360, width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {sesionTourPaso < tourSlides.length - 1 ? (
                ...
                <button onClick={completarPrimeraSesion} style={{ background: 'var(--dark)', color: '#fff', border: 'none', borderRadius: 16, padding: '16px 40px', fontSize: 15, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>Hablar con {coachName} ✦</button>
              )}
  ...
  const handleCheckout = async (planId) => {
    try {
      const res = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ planId, userId: user.id, userEmail: user.email, vertical: config.vertical }) })
      const data = await res.json()
    ...
    <div style={{ minHeight: '100vh', background: '#0f0d0b', paddingBottom: 80, position: 'relative' }}>
        ...
        <div>
          ...
          <div style={{ padding: '20px' }}>
            ...
            <button onClick={() => { setChatMsgs([{ r: 'u', t: `Quiero hablar sobre el módulo "${activeModulo.titulo}". ¿Cómo me ayudas a aprovecharlo mejor?` }]); navigate('clara') }} style={{ width: '100%', background: 'transparent', border: '1px solid var(--gold-light)', color: 'var(--text)', padding: '14px 24px', borderRadius: 30, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
              Hablar con {coachName} sobre este módulo ✦
            </button>
                  ...
                  {recomendado && (
                    <div className="mo-feat-reco">
                      ...
                      <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.55, flex: 1 }}>
                        {recomendado.descripcion || `${coachName} pensó que esto te iba a servir. Son ${recomendado.numero_semanas || 4} semanas de contenido corto.`}
                      </p>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
                        <img src={coachImg} alt={coachName} style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover' }} />
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 500 }}>{coachName}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)' }}>
                            {config.coach.tag.replace('Coach 360 ', '')} · {recomendado.numero_semanas || 4} semanas
                          </div>
                        </div>
                    ...
                    <div key={m.id} onClick={() => abrirModulo(m)} style={{ background: 'var(--ink-2)', border: '1px solid var(--line)', borderRadius: 16, overflow: 'hidden', opacity: locked ? 0.6 : 1, cursor: 'pointer' }}>
                      ...
                      <div style={{ padding: 18 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 10, borderTop: '1px solid var(--line)' }}>
                          <img src={coachImg} alt={coachName} style={{ width: 22, height: 22, borderRadius: '50%', objectFit: 'cover' }} />
                          <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>por {coachName}</span>
      ...
      {view === 'ritual' && !activeTest && (() => {
        ...
        const items = [
          ...
          { n: 'II',  t: 'Leer la carta del día', d: `Algo que ${coachName} escribió para ti a partir del test.`,       time: '2 min',  action: 'Abrir carta',  img: 'https://images.unsplash.com/photo-1518622358385-8ea7d0794bf6?w=600&q=80&auto=format&fit=crop' },
      ...
      {activeTest && !testResult && (() => {
        ...
        return (
          <div className="dir-ritual" data-v={config.dataV} style={{ position: 'fixed', inset: 0, zIndex: 1000, overflowY: 'auto', background: 'var(--bg)', color: 'var(--text)', display: 'flex', flexDirection: 'column' }}>
            <style>{`
            ...
            <div className="at-wrap">
              <div className="at-header">
                <img src={coachImg} alt={coachName} style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                ...
                <div className="at-right">
                  <div style={{ padding: 22, background: 'var(--ink-2)', border: '1px solid var(--line)', borderRadius: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                      <img src={coachImg} alt={coachName} style={{ width: 34, height: 34, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                  ...
                  <div style={{ padding: 14, background: 'var(--ink-3)', border: '1px solid var(--line)', borderRadius: 12, fontSize: 11, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)', lineHeight: 1.5, textAlign: 'center' }}>
                    tus respuestas solo las ve {coachName}. cifrado end-to-end.
                  </div>
                </div>
              </div>
            </div>
      ...
      {testResult && (() => {
        ...
        return (
          <div className="dir-ritual" data-v={config.dataV} style={{ position: 'fixed', inset: 0, zIndex: 1000, overflowY: 'auto', background: 'var(--bg)', color: 'var(--text)' }}>
            <style>{`
              ...
              <div className="tr-grid">
                ...
                <div className="tr-cards">
                  ...
                  <div style={{ padding: 26, background: 'linear-gradient(135deg, var(--v-tint), var(--ink-2) 80%)', border: '1px solid color-mix(in oklab, var(--v-primary) 30%, var(--line))', borderRadius: 18 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                      <img src={coachImg} alt={coachName} style={{ width: 34, height: 34, borderRadius: '50%', objectFit: 'cover' }} />
                  ...
                  <button onClick={conversarConClara} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', background: 'var(--v-primary)', color: '#0a0c0e', fontWeight: 600, fontSize: 15, cursor: 'pointer', fontFamily: 'var(--font-body)', marginTop: 4 }}>
                    Conversar con {coachName} sobre esto →
                  </button>
        ...
        <div className="dir-ritual" data-v={config.dataV} style={{ background: 'var(--bg)', color: 'var(--text)', minHeight: '100vh' }}>
          ...
          <div className="ch-wrap">
              ...
              <div className="ch-header">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: 'var(--font-display)', fontSize: 20, letterSpacing: '-0.02em' }}>{coachName}</span>
                ...
                {chatMsgs.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 12, flexDirection: m.r === 'u' ? 'row-reverse' : 'row', alignItems: 'flex-end' }}>
                      {m.r === 'a' && (
                        <img src={coachImg} alt={coachName} style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                      )}
                      <div style={{
                        maxWidth: 520,
                        padding: '14px 18px',
                        ...
                        {esPrimerMsgCoach && (
                          <div className="eyebrow" style={{ color: 'var(--v-primary)', marginBottom: 8 }}>✦ {coachName} recuerda</div>
                        )}
                        {m.t}
                        {hora && (
              ...
              <div className="ch-input-wrap">
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, padding: '10px 14px', background: 'var(--ink-2)', border: '1px solid var(--line-strong)', borderRadius: 20 }}>
                  ...
                  <textarea
                    ...
                    placeholder={`Escríbele a ${coachName}...`}
                    rows={1}
            ...
            <aside className={`ch-context ${contextoAbierto ? 'open' : ''}`}>
              ...
              <div>
                <div className="eyebrow" style={{ marginBottom: 10 }}>✦ {coachName} recuerda</div>
              ...
              <div>
                ...
                {[
                  { t: 'Ejercicio de respiración', sub: `3 min · sugerido por ${coachName}` },
                ...
                <div className="pf-col">
                  ...
                  <div style={{ padding: 24, borderRadius: 20, background: 'linear-gradient(140deg, var(--v-tint), var(--ink-2) 75%)', border: '1px solid color-mix(in oklab, var(--v-primary) 25%, var(--line))' }}>
                    ...
                    <div className="pf-coach-card-inner">
                      <img src={coachImg} alt={coachName} className="pf-coach-img" style={{ borderRadius: 18, objectFit: 'cover' }} />
                      <div>
                        <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, letterSpacing: '-0.025em' }}>{coachName}</div>
                  ...
                  <div style={{ padding: 24, borderRadius: 18, background: 'var(--ink-2)', border: '1px solid var(--line)' }}>
                    <div style={{ marginBottom: 20 }}>
                      <div className="eyebrow" style={{ marginBottom: 6 }}>Tono de {coachName}</div>
      ...
      {mostrarPopupPostTest && testResult && (
        <div className="dir-ritual" data-v={config.dataV} onClick={() => cerrarPopupPostTest()} style={{ position: 'fixed', inset: 0, zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(10,12,14,.78)', padding: 16 }}>
          <style>{`
          ...
          <div className="pop-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pop-img">
              <img src={coachImg} alt={coachName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ...
            <div className="pop-body">
              ...
              {plan === 'free' ? (
                ...
                <button onClick={() => cerrarPopupPostTest(() => navigate('clara'))} style={{ padding: 14, borderRadius: 12, border: 'none', background: 'var(--v-primary)', color: '#0a0c0e', fontWeight: 600, fontSize: 15, cursor: 'pointer', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontFamily: 'var(--font-body)' }}>
                  Hablar con {coachName} ahora <Icon.arrow s={14} />
                </button>
              )}
      ...
      {!activeTest && !testResult && !activeHerramienta && !['clara', 'equilibrio', 'herramienta_activa', 'planes'].includes(view) && (
        <div className="dir-ritual" data-v={config.dataV}>
          <style>{`
            .bnav {
          ...
          <div className="bnav">
            {[
              ...
              { label: coachName,    I: Icon.dots,    active: view === 'clara',       action: () => navigate('clara') },
              { label: 'Módulos',    I: Icon.book,    active: view === 'modulos' || view === 'modulo',  action: () => navigate('modulos') },
